...
function PurchaseReceiptDetail() {
  ...
  const generatePDF = () => {
    ...
    doc.text(receipt.receiptDate ? new Date(receipt.receiptDate).toLocaleDateString() : 'N/A', 130, 45);
    doc.text(receipt.receivedBy?.name || 'N/A', 60, 55);
    
    // Create table for items with enhanced styling
    const tableColumn = ["Product", "Received Qty", "Batch #", "Expiry Date", "Unit Price", "Comments"];
    const tableRows = [];
    ...
    const statusColor = receipt.qualityCheck?.passed ? [0, 128, 0] : [200, 0, 0];
    ...
    doc.text("Apothecary Shop | Generated on: " + new Date().toLocaleString(), 14, finalY + 7);
  ...
  if (loading) return <div className="text-center p-4">Loading purchase receipt data...</div>;
  if (error) return <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">{error}</div>;
  if (!receipt) return <div className="text-center p-4">Purchase receipt not found</div>;
  
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-6">Purchase Receipt: {receipt.receiptNumber}</h1>
      
      <div className="bg-gray-50 p-4 rounded-lg mb-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <p className="text-sm text-gray-500">PO Number</p>
            <p className="font-medium">{receipt.purchaseOrder?.poNumber || 'N/A'}</p>
          </div>
          <div>
            <p className="text-sm text-gray-500">Receipt Date</p>
            <p className="font-medium">{receipt.receiptDate ? new Date(receipt.receiptDate).toLocaleDateString() : 'N/A'}</p>
          </div>
          <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          <div className="mb-4">
            <span className={`px-2 py-1 rounded text-xs font-semibold ${receipt.qualityCheck?.passed ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          <p className="whitespace-pre-wrap">{receipt.notes || 'No additional notes'}</p>
        </div>
      </div>
